...
if ($id_funcionario) {
    $sql = "
        SELECT DATE_FORMAT(data_exame, '%Y-%m') AS mes,
               COUNT(*) AS qtd
...
$detalhes = [];
if ($id_funcionario) {
    $stmt = $pdo->prepare("SELECT * FROM exame_preventivo WHERE id_funcionario = :id ORDER BY data_exame DESC LIMIT 50");
    $stmt->execute([':id'=>$id_funcionario]);
...
$json_exames_mes = json_encode($exames_mes);
$json_labels_dias = json_encode($labels_dias);
$json_exames_dia = json_encode($exames_dia);
...
<title>Exames Preventivos — RadyX</title>
...
<div class="detail-page-container">
  ...
  <div class="detail-header">
    ...
    <div class="detail-title">
      <h1>EXAMES PREVENTIVOS</h1>
      <p style="color:#e6dbff; margin-top:6px;">Resumo dos seus exames</p>
    </div>
  </div>

  <div class="detail-body">
    <p>Abaixo estão seus registros de exames preventivos e gráficos relacionados.</p>

    <div class="stats-row">
      <div class="stat-card">
        <h4>Exames (últimos meses)</h4>
        <div class="value"><?= array_sum($exames_mes) ?></div>
      ...
      <div class="stat-card">
        ...
        <div class="value">
          <?php if(!empty($detalhes)):
                  echo htmlspecialchars($detalhes[0]['data_exame']);
              else: echo '—';
              endif; ?>
        </div>
    ...
    <div class="chart-card">
      <h3 style="color:var(--lilas);">Exames por mês</h3>
      <canvas id="chartMes" style="max-height:300px;"></canvas>
    </div>

    <div class="chart-card">
      <h3 style="color:var(--lilas);">Você vs Empresa</h3>
      <canvas id="chartComp" style="max-height:360px;"></canvas>
    </div>

    <div class="table-wrap">
      <h3 style="color:var(--lilas); margin-bottom:.6rem;">Registros</h3>
      <table class="report-table">
        ...
        <tbody>
          ...
          <?php else: foreach($detalhes as $d): ?>
            <tr>
              <td><?= htmlspecialchars($d['tipo_exame']) ?></td>
              <td><?= htmlspecialchars($d['data_exame']) ?></td>
              <td><?= htmlspecialchars($d['resultado_exame']) ?></td>
            </tr>
    ...
    <div style="text-align:right; margin-top:2rem;">
      <a href="form-exames-preventivos.php" class="btn-primary" style="background-color:#7c3aed;color:white;padding:12px 28px;border-radius:12px;font-size:1.1rem;text-decoration:none;">
        Novo Registro
      </a>
    </div>
...
const examesMes = <?= $json_exames_mes ?>;
const dias = <?= $json_labels_dias ?>;
const examesDia = <?= $json_exames_dia ?>;
...
new Chart(document.getElementById('chartMes'), {
  type:'bar',
  data:{ labels:meses, datasets:[{ label:'Exames', data:examesMes, backgroundColor:'#7c3aed' }] },
  options: {
        responsive: true,
        scales: {
            y: {
...
new Chart(document.getElementById('chartDia'), {
  type:'line',
  data:{ labels:dias, datasets:[{ label:'Registros', data:examesDia, borderColor:'#10b981', tension:0.25 }] },
  options: {
        responsive: true,
        scales: {
            y: {
